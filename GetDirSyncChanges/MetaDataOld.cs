    ...
    static public class Metadata
    {
        ...
        public static void GetMetaData(LdapConnection ldapConnection, string objectDN)
        {
            ...
            if (entry.Attributes.Contains("msDS-ReplAttributeMetaData;binary"))
            {
                ...
                {
                    DecodeMetaData("attribute", byteArray);
                }
            }
            ...
            if (entry.Attributes.Contains("msDS-ReplValueMetaData;binary"))
            {
                ...
                {
                    DecodeMetaData("value", byteArray);
                }
            }
        }
        ...
        static void DecodeMetaData(string dataType, byte[] byteArray)
        {
            ...
            {  
                ...
                // since this is unicode need to do this way
                string attribute = "";
                for (int i = attributeOffset; i < byteArray.Count(); i = i + 2)
                {
                    if (byteArray[i] == 0) break;
                    attribute = attribute + Convert.ToChar(byteArray[i]).ToString();
                }
                ...
                if (DSAOffset != 0)
                {
                    for (int i = DSAOffset; i < byteArray.Count(); i = i + 2)
                    {
                        if (byteArray[i] == 0) break;
                        dsa = dsa + Convert.ToChar(byteArray[i]).ToString();
                    }
            ...
            if (dataType == "value")
            {
                ...
                string objectDN = "";
                for (int i = objectDNOffset; i < byteArray.Count(); i = i + 2)
                {
                    if (byteArray[i] == 0) break;
                    objectDN = objectDN + Convert.ToChar(byteArray[i]).ToString();
                }
